...
STRINGS = {
    LANG_RU: {
        'window_title': 'T&P — TRANSAS USER CHART',
        'header': 'T&P  |  TRANSAS USER CHART',
        ...
        'route_steps': (
            ...
            '8. Немного ждём, папка с .aiz файлом откроется автоматически'
        ),
        'world_info': (
            'Собрать .aiz со всеми T&P нотисами из каталога ADC.\n'
            'T&P World.aiz сохраняется в папку Output.'
        ),
        ...
        'status_route_updated': 'Route_T&P.aiz обновлён ({count} objects)',
        ...
        'status_world_ready': 'Мировая карта готова ({count} objects).',
    ...
    LANG_EN: {
        ...
        'route_steps': (
            ...
            '8. Wait a moment — the folder with the .aiz file will open automatically'
        ),
        'world_info': (
            'Build .aiz with all T&P notices from the ADC catalogue.\n'
            'T&P World.aiz is saved to the Output folder.'
        ),
        ...
        'status_route_updated': 'Route_T&P.aiz updated ({count} objects)',
...
def lang_code_to_label(code):
    return LANG_LABELS.get(code, LANG_LABELS[LANG_RU])


def label_to_lang_code(label):
...
def t(key, lang=None, **kwargs):
    ...
    text = STRINGS.get(lang, STRINGS[LANG_RU]).get(key, STRINGS[LANG_RU].get(key, key))
    if kwargs:
